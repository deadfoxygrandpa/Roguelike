...
display state =
    let (w, h) = (state.level.size.width * xScale, state.level.size.height * yScale)
        ...
        pl = (xOffset state.player.location.x, yOffset state.player.location.y)
        ...
        pg = collage w h [player]
    in  flow down [bg, pg, collage w h [moveX -120 player], asText state.player, asText pl]
